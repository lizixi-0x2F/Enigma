...
我们进一步对Enigma模型进行了优化和功能扩展，提升了整体性能和生成质量：

### 1. 高级生成策略

增强了模型的生成功能，提供更高质量的输出文本：

- **Beam Search生成**：实现了带有多样性惩罚的beam search算法，平衡了生成多样性和连贯性
- **Contrastive Search**：添加了基于对比搜索的解码策略，减少重复性并提高长文本生成质量
- **温度退火**：在Gumbel-Sinkhorn软置换中实现了温度退火机制，从1.0逐步降低到0.1
- **Flow模型集成**：完整集成了EnigmaFlow生成模型，支持隐空间采样和密度估计

### 2. 接口优化与代码清理

为提高代码质量和用户体验，进行了全面的接口优化：

- **模块化组织**：将Flow和Gumbel-Sinkhorn相关代码整合进主模型架构
- **统一参数接口**：通过命令行参数统一控制所有训练和生成功能
- **代码清理**：移除过时的实验性脚本，整合功能到核心模块中
- **混合精度训练**：明确指定fp16精度训练，相比fp32提供2-3倍的速度提升

### 3. 训练流程优化

全面升级训练流程，提高训练效率和稳定性：

- **梯度累积步数**：从2增加到4，在保持内存效率的同时提高有效批量大小
- **学习率微调**：从1e-3调整为8e-4，减少训练不稳定性
- **FP16精度训练**：默认启用FP16混合精度训练，显著提高计算效率
- **增强数据加载**：优化数据流水线，支持更大规模数据集训练

完整的最新训练命令如下：

```bash
# 使用最新优化配置进行训练
python -m scripts.train_full_data \
  --epochs 15 \
  --data wiki-full-zh \
  --save-dir checkpoints_full \
  --d-model 768 \
  --batch-size 32 \
  --grad-accum-steps 4 \
  --num-rev-blocks 6 \
